...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .motiv-val { width: 32px; text-align: right; font-size: 12px; font-weight: 600; }
  .comment { background: #f8fafc; border-left: 3px solid #6366f1; padding: 10px 14px; margin-bottom: 8px; font-size: 12px; color: #475569; line-height: 1.5; }
...
${[['Зарплата', motivation.salary], ['Признание', motivation.recognition], ['Развитие', motivation.development], ['Стабильность', motivation.stability], ['Команда', motivation.team], ['Автономия', motivation.autonomy]]
  .map(([label, val]) => `<div class="motiv-row"><div class="motiv-label">${label}</div><div class="motiv-bar-bg"><div class="motiv-bar" style="width:${((val as number)/5)*100}%"></div></div><div class="motiv-val">${val}</div></div>`).join('')}
${aiReportHtml}
${commentsHtml}
  ...
  return (
    <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 px-6 py-4 border-b border-slate-100 bg-slate-50">
        ...
        {loading && <span className="ml-auto text-xs text-indigo-500 animate-pulse font-medium">Анализирую данные...</span>}
